...
if ! command -v go >/dev/null 2>&1; then
    echo "Go 1.23 or newer is required to build ASRL and DroidForge." >&2
    exit 1
fi
...
if ((GO_MAJOR < 1 || (GO_MAJOR == 1 && GO_MINOR < 23))); then
    echo "Go 1.23 or newer is required to build ASRL and DroidForge (found $(go env GOVERSION))." >&2
    exit 1
fi
